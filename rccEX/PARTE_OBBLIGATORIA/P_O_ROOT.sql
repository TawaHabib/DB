/*A. DA GCP*/
/*b. Creare uno schema di nome PROVA / DA GCP*/
CREATE SCHEMA IF NOT EXISTS PROVA;
/*C. ACCESSO REMOTO*/
/*d. (da WB) Popolare PROVA con le table dello stesso schema usato sul PC */
--
-- script per creare e popolare le tabelle del database PROVA
--
--    versione 2
--                 (rispetto completo dei vincoli relazionali)
--
-- lo script usa la sintassi SQL supportata da MYSQL ver 5.5 community
-- ed e' stato verificato su un'installazione WIN XP MySQL ver 5.5.
-- lo script presuppone che sia stato creato il database PROVA
-- eseguendo il comando CREATE DATABASE PROVA
--
-- le parti non standard dei comandi sono
--   1) la clusola IF EXISTS del comando DROP TABLE
--   2) la clausola ENGINE=InnoDB che e' specifica di MySQL
--      senza la quale non e' possibile usare i comandi di
--      definizione dei vincoli di integrita'Â  referenziale
--      FOREIGN KEY
--   3) il tipo dati TIME (cosi' come quello DATE) sono un'estensione MySQL
--      del tipo dati DATETIME dello standard
--
...
--
-- Le tabelle DIPART e DIPENDENTI sono  definite con chiave primaria
-- e con un vincolo esplicito di integrita'Â  referenziale fra la chiave esterna DIPENDENTI(NUM_DIPART) e
-- DIPART(NUM_DIPART)
-- All'interno di DIPENDENTI l'attributp SUP e' definito sullo stesso dominio della chiave primaria
-- MATRICOLA, e puÃƒÂ² quindi essere considerato chiave "esterna" della stessa tabella
-- con un opportuno vincolo di integritÃƒÂ  referenziale. Il vincolo pero' impone forti limitazioni
-- a come si inseriscono le righe nella tabella (perche' ?), quindi per creare il database e' piu'
-- comodo creare la tabella SENZA il vincolo, caricare le tuple in modo che i dati rispettino
-- il vincolo, e successivamente MODIFICARE lo schema della tabella, con un opportuno comando
-- di ALTER TABLE, con il quale si aggiunge il vincolo
--
...
INSERT INTO SPED_DETTAGLI VALUES
        ('SP08','F317','P008','PR58');
/*e. (da WB) Creare un nuovo utente denominato BDXXXXXX dove XXXXXX è la matricola*/
CREATE USER 'BD481357'IDENTIFIED BY '12345678';
/*f. (da WB) Autorizzare l’utente BDXXXXXX ad accedere SOLO in lettura allo schema PROVA, ma concedergli altre opportune autorizzazioni perché possa svolgere i passi successivi.*/
GRANT SELECT ON PROVA.* TO BD481357;
GRANT SELECT, INSERT, CREATE, references ON *.* TO BD481357;
